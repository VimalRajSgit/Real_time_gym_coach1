...
# Groq API configuration - Added from second code
def get_llama_response(prompt, system_prompt="You are a gym coach. Provide tips during specific exercises, like for squats say to keep the core engaged, for pull-ups ensure proper posture, and offer continuous motivation. Keep responses short and actionable."):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.workout_plan:
                    ...
                    # Replace with Groq LLaMA
                    prompt = f"Starting {self.target_reps} {self.current_exercise}s!"
        ...
        while self.is_running and self.cap and self.cap.isOpened():
            ...
            if self.current_exercise and self.frame_skip == 0:
                ...
                if current_count > previous_count:
                    ...
                    if remaining == 0:
                        # Replace with Groq LLaMA
                        prompt = f"Done! {self.target_reps} {self.current_exercise}s!"
                        response = get_llama_response(prompt)
                        self.add_chat_message("Coach", response)
                        ...
                        if self.workout_plan:
                            ...
                            # Replace with Groq LLaMA
                            prompt = f"Next: {self.target_reps} {self.current_exercise}s!"
                            response = get_llama_response(prompt)
                            self.add_chat_message("Coach", response)
                        else:
                            self.current_exercise = None
                            # Replace with Groq LLaMA
                            prompt = "Workout complete! How did you feel?"
                            response = get_llama_response(prompt)
                            self.add_chat_message("Coach", response)
                    else:
                        # Replace with Groq LLaMA
                        prompt = f"Great! {remaining} left!"
                        response = get_llama_response(prompt)
                        self.add_chat_message("Coach", response)
